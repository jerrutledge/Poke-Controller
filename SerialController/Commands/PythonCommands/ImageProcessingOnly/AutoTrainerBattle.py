...
class AutoTrainerBattle(ImageProcPythonCommand, ResetGame):
    ...
    def do(self):
        print(
            "AUTO BATTLE: use_swords_dance="
            + str(self.use_swords_dance)
            + " "
            + str(self.times_set_up)
            + " times"
        )
        while True:
    ...
    def bestAttack(self, givenPokemon=None, turn=2):
        ...
        attack_text.append(
            self.getText(tb[0]["top"], tb[0]["bottom"], move_lr[0], move_lr[1])
        )
        attack_text.append(
            self.getText(tb[1]["top"], tb[1]["bottom"], move_lr[0], move_lr[1])
        )
        attack_text.append(
            self.getText(tb[2]["top"], tb[2]["bottom"], move_lr[0], move_lr[1])
        )
        attack_text.append(
            self.getText(tb[3]["top"], tb[3]["bottom"], move_lr[0], move_lr[1])
        )

        # if we are already dynamaxed, move text appears in white
        # OCR processing must invert the image to get a proper reading
        if (
            "Max" in attack_text[0]
            or "Max" in attack_text[1]
            or "Max" in attack_text[2]
            or "Max" in attack_text[3]
        ):
            dynamaxed = True
            attack_text.clear()
            attack_text.append(
                self.getText(
                    tb[0]["top"], tb[0]["bottom"], move_lr[0], move_lr[1], inverse=True
                )
            )
            attack_text.append(
                self.getText(
                    tb[1]["top"], tb[1]["bottom"], move_lr[0], move_lr[1], inverse=True
                )
            )
            attack_text.append(
                self.getText(
                    tb[2]["top"], tb[2]["bottom"], move_lr[0], move_lr[1], inverse=True
                )
            )
            attack_text.append(
                self.getText(
                    tb[3]["top"], tb[3]["bottom"], move_lr[0], move_lr[1], inverse=True
                )
            )
        ...
        for i in range(4):
            ...
            if cur_pp <= 0:
                ...
                "Super effective" in attack_text[i]
                or ("Super e" in attack_text[i])
                or ("Super" in attack_text[i] and "Max" in attack_text[i])
            ):
            ...
            if (
                (not (dynamax or dynamaxed))
                and cur_move
                and cur_move["healing"]
                and self.current_hp < self.max_hp / 2.1
            ):
                ...
                if (
                    givenPokemon
                    and "stats" in givenPokemon
                    and len(givenPokemon["stats"]) > 5
                ):
                ...
                if dynamax or dynamaxed:
                    ...
                    if (
                        cur_move["moveType"] == "FIGHTING"
                        and cur_move["moveCategory"] == "Physical"
                        or cur_move["moveType"] == "POISON"
                        and cur_move["moveCategory"] == "Special"
                    ):
                        # 1.5 = avg(1,1.5,2)
                        effectiveness *= 1.5
                    # if you lower their defense
                    elif (
                        cur_move["moveType"] == "GHOST"
                        and cur_move["moveCategory"] == "Physical"
                        or cur_move["moveType"] == "DARK"
                        and cur_move["moveCategory"] == "Special"
                    ):
                        # 1.5 = avg(1,1.5,2)
                        effectiveness *= 1.5
                    ...
                    effectiveness *= (
                        cur_move["power"] * accuracy / cur_move["turnsTaken"]
                    )
